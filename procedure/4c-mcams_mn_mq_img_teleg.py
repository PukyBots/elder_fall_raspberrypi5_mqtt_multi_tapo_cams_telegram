...
vosk_model = Model(
    "/home/tce/tapo_cam_rpi5/vosk-model-small-en-us-0.15"
)
...
for url in RTSP_URLS:
    ...
    if cap.isOpened():
        ...
        print(f"Offline at startup: {url}")
...
def process_wellness_check():
    ...
    if hello_detected or hand_raised_detected:
    ...
    if (
        ...
        speak(
            "Hello. Please say hello or raise your hand."
        )
...
while True:
    ...
    for cam_index, cap in enumerate(caps):
        ...
        if cap is None:
            ...
            if (
                time.time()
                - state["last_reconnect_attempt"]
                > 15
            ):

                state["last_reconnect_attempt"] = time.time()

                reconnect_camera_async(cam_index)
        ...
        if not ret:
            ...
            print(
                f"[CAM {cam_index+1}] Stream lost"
            )

            if time.time() - state["last_reconnect_attempt"] > 10:
        ...
        if keypoints is not None:
            ...
            state["last_seen"] = datetime.now().strftime(
                "%H:%M:%S"
            )
